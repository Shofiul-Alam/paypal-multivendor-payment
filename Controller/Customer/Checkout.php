...
class Checkout extends \XLite\Controller\Customer\Checkout implements \XLite\Base\IDecorator
{
    ...
    /**
     * My Custom Development start Here
     */

    protected function getExpressCheckoutCompletedCustomerDetails() {
    ...
    protected function doConfirmPaymentAuthorization() {
        ...
        $data = $this->getExpressCheckoutCompletedCustomerDetails();

        $processor = $paymentMethod->getProcessor();
        $token = $processor->doConfirmAuthorizationExpressCheckout($paymentMethod, $data);

        return $token;
    }

    protected function doCapturePaymentExpressCheckout($transaction) {
        ...
        $state = $processor->doExpressCheckoutCapture($transaction);

        return $state;
    }

    protected function expressCheckoutCancelReturn() {
    ...
    {
        if(\XLite\Core\Request::getInstance()->action == 'express_checkout_return' && \XLite\Core\Request::getInstance()->cancel ==1 ) {

            $this->expressCheckoutCancelReturn();

        }
        else {


            $token = $this->doConfirmPaymentAuthorization();
            ...
            $transaction->setPayerId($request->getData()->PayerId);
            $transaction->setInitial(true);

            //We have to set transaction data successfully here.

//
//
//        $backendTransaction = new \XLite\Model\Payment\BackendTransaction();
//
//
//
//
//
//
//        $capture = $this->doCapturePaymentExpressCheckout($backendTransaction);
            ...
            if (isset($request->cancel)) {
                ...
                $transaction->setToken($token['token']);
                $transaction->setTranId($token['trnsId']);
                $transaction->setPayerId($buyerData['PAYERID']);
                ...
                if (empty($buyerData)) {
                    ...
                    $transactionId = $this->getCart()->getFirstOpenPaymentTransaction()->getTransaction_id();

                    $backendTransaction = \XLite\Core\Database::getRepo('\XLite\Model\Payment\BackendTransaction')->findBy(array('payment_transaction' => 26));

                    if (\XLite\Core\Session::getInstance()->ec_ignore_checkout) {


//                    $this->doCapturePaymentExpressCheckout($transaction);
