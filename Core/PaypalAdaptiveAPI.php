...
class PaypalAdaptiveAPI extends \XLite\Module\CDev\Paypal\Core\AAPI
{
    ...
    protected function getReceivers(\XLite\Model\Order $order)
    {
        ...
        $receivers[0] = array(
            'amount'    => $order->getCurrency()->roundValue($order->getTotal()),
    ...
    /**
     * My Custom Preaproval Request is building here.....
     */

    protected function getApprovalReceivers(\XLite\Model\Order $order)
    {
        ...
        if($orderAdditional && $orderAdditional->getOrderBondSubtotal()) {
            $bond = $orderAdditional->getOrderBondSubtotal();
        }
...
//        }
    ...
    public function doPayCall(\XLite\Model\Order $order, $cancelURL, $returnURL, $ipnNotificationUrl)
    {
        ...
        if($orderAdditional && $orderAdditional->getOrderBondSubtotal()) {
            $maxAmount = $orderAdditional->getOrderBondSubtotal();
        }

        //my testing Section start here

        $approvalTo = array_values($this->getApprovalReceivers($order));
        ...
        $paramsX = array(
            ...
            'cancelUrl'             => $cancelURL,
            'returnUrl'             => $returnURL,
            'startingDate'          => "2017-04-28T10:45:52Z",
            'endingDate'            => "2017-05-28T10:45:52Z",
            'maxNumberOfPayments'   => 20,
            'maxAmountPerPayment'   => 850.00,
            'maxTotalAmountOfAllPayments' => 900,
    ...
    protected function getApiURL($method)
    {
        ...
        switch ($method) {
            case 'Pay':
                ...
                break;
            case 'Preapproval':
                $url .= 'AdaptivePayments/Preapproval';
                break;
