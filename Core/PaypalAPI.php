...
//            : 'Sale';
    ...
    protected function getItems($order)
    {
        ...
        /**My custom Developmetn */
        $orderAdditional = $order->getOrderAdditional();
        /** @var \XLite\Model\Currency $currency */
        $currency = $order->getCurrency();

        $bondSubtotal = $currency->roundValue($orderAdditional->getOrderBondSubtotal());
        $result['L_PAYMENTREQUEST_0_AMT' . $index] = $bondSubtotal;
        $result['L_PAYMENTREQUEST_0_NAME' . $index] = 'Total Bond Mone: Authorize Only';
        $result['L_PAYMENTREQUEST_0_NUMBER' . $index] = 'TMH-B2017';
        $result['L_PAYMENTREQUEST_0_QTY' . $index] = 1;

        $itemsSubtotal += $bondSubtotal * 1;
     ...
     */
    public function convertMassPaymentParams($transaction, $token, $payerId)
    {
        ...
        $secondary_receivers = $this->getSecondaryReceivers($order);
        ...
        $totalIncludingBondMoney = ($orderTotal + $bondSubtotal);
        ...
        $index = 0;
        foreach($secondary_receivers as $receiver) {

            $receiver = array(
                'L_EMAIL'.$index => $receiver['email'],
                'L_AMT'.$index => $receiver['amount'],
                'L_NOTE'.$index => 'Hello Not is going to take place here'
            );

            $params = array_merge($params, $receiver);
...
//        if (abs($totalIncludingBondMoney - $items['PAYMENTREQUEST_0_ITEMAMT'] - $taxAmt - $shippingCost) > 0.0000000001) {
//            $correction = $totalIncludingBondMoney - $items['PAYMENTREQUEST_0_ITEMAMT'] - $taxAmt - $shippingCost;
...
//        $params += $items;
    ...
    /**
     * My custom Development End Here
     */
    ...
    public function convertSetExpressCheckoutParams($order)
    {
        ...
        $params = array(
            ...
            'ALLOWNOTE'                      => 1,
            'PAYMENTREQUEST_0_AMT'           => $totalIncludingBondMoney, //My Custom Development
            'PAYMENTREQUEST_0_PAYMENTACTION' => 'Authorization', //$this->getPaymentAction(), My Custom Development
        ...
        if (abs($totalIncludingBondMoney - $items['PAYMENTREQUEST_0_ITEMAMT'] - $taxAmt - $shippingCost) <= 0.0000000001) {
            ...
            $itemsAmt = $totalIncludingBondMoney - (float) $shippingCost;
    ...
    public function convertDoExpressCheckoutPaymentParams($transaction, $token, $payerId)
    {
        ...
        $params = array(
            'TOKEN'                          => $token,
            'PAYERID'                        => $payerId,
            'PAYMENTREQUEST_0_AMT'           => $totalIncludingBondMoney,
